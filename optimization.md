...
(`faustovrz/RTIGER`, branch `optimize-julia-core`). The goal is to make RTIGER
usable on **large populations** — on the order of **1400 samples × ~50 000
...
## 4. Memory optimization — streaming M-step
...
## 5. Progress logging (opt-in, default off)
...
All benchmark/validation code is committed under `agent/` (no hidden temp dirs):

- `agent/scale_check/` — full-resolution real-data checks (`README.md`,
  scripts `00`–`16`): `03_decode_compare.jl` (270 k decode = 100%),
  `11_rinit_ab.jl` (15 k bit-identical A/B), `14_fullfit_compare.sh`
  (270 k optimized-vs-original head-to-head), `15_progress_log.jl` /
  `16_progress_log_R.R` (progress-log tests).
- `agent/bench/` — `harness.jl` (equivalence harness + committed baseline),
  `mem_bench.jl` + `scaling_after_streaming.sh` (peak-RSS scaling sweep),
  `profile_phases.jl`.

The upstream original is materialized for A/B comparison via
`agent/scale_check/00_materialize_baseline.sh`.
